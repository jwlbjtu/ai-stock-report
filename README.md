...
- **新闻归因**：Alpha Vantage `NEWS_SENTIMENT` 为主源，Google News RSS 兜底；**60 分钟时间对齐在代码里完成**（只把异动时刻附近的新闻喂给模型）。
- **LLM 复盘**：DeepSeek-chat 生成结构化 JSON（资金广度 / 关联解读 / 形态点评 / 连续性复盘 / 风险提示），带**跨日记忆**（结合昨日结论看延续 vs 反转）。
- **推送降级链**：WxPusher → Telegram → 落盘，主通道失败自动切备，多收件人支持。
- **报告网页**：静态 HTML（内联 CSS，移动端优先），自动生成历史报告列表 `index.html`。
